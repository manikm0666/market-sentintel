...
# --- 3. UI STYLING (The Look You Built) ---
st.markdown("""
    <style>
    ...
    /* Mobile Fixes: 2x2 Grid + Small Star */
...
# --- 5. SIDEBAR (Persistent Watchlist) ---
...
# --- 6. MAIN CONTENT ---
try:
    # DATA STABILITY FIX: We fetch and then immediately clean the columns
    ticker_obj = yf.Ticker(TICKER)
    df = ticker_obj.history(period=time_map[selected_label], 
                            interval="1m" if selected_label=="1D" else "1d")
    
    if not df.empty:
        # THE FIX: If yfinance sends a Multi-Index, we force it to a single level
        ...
        st.markdown(f"### 🔮 AI PROJECTION: <span style='color:#00FF41;'>${pred:.2f}</span>", 
                    help="Linear regression trend line based on the chart interval.", unsafe_allow_html=True)

        # 2x2 Metric Grid with Tooltips restored
        m1, m2 = st.columns(2)
        m3, m4 = st.columns(2)
        m1.metric("Market Price", f"${curr_p:.2f}", help="Latest price.")
        m2.metric("Trend Velocity", f"{slope:+.4f}", help="Price movement strength.")
    ...
    st.info("Check ticker spelling or wait a moment before searching again.")